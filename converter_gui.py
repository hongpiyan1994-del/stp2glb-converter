...
def convert_stp_to_glb(stp_path, output_glb_path, blender_exe, status_callback):
    ...
    # ─── 诊断：打印所有可用的 addon_modules ───
    lines = [
        "import sys\n",
        "print('SCRIPT_START')\n",
        "import bpy, addon_utils\n",
        "print('SCRIPT_AFTER_IMPORTS')\n",
        "# List all available addon module names containing 'step'\n",
        "step_candidates = [m for m in addon_utils.module_keys() if 'step' in m.lower()]\n",
        "print('STEP_ADDON_CANDIDATES:', step_candidates)\n",
        "# Try to enable each candidate until one works\n",
        "step_addon = None\n",
        "for m in step_candidates:\n",
        "    try:\n",
        "        ok, handle = addon_utils.enable(m, persistent=True)\n",
        "        print('TRY_ENABLE', m, '->', ok)\n",
        "        if ok:\n",
        "            step_addon = m\n",
        ...
        "# Also add scripts/addons to path if present\n",
        "import os\n",
        "addon_path = os.path.join(bpy.utils.resource_path('USER'), 'scripts', 'addons')\n",
        "if os.path.exists(addon_path):\n",
        "    if addon_path not in sys.path:\n",
        "        sys.path.append(addon_path)\n",
        "    print('ADDON_PATH:', addon_path)\n",
        "else:\n",
        "    print('NO USER ADDON PATH')",
